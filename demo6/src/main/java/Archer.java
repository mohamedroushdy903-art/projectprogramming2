...
public class Archer extends com.example.game.Fighter {
    ...
    public com.example.game.Projectile shoot(double dirX, double dirY, Pane root) {
        ...
        double px = getX() + width / 2.0 + dx * (width / 2.0 + 3);
        double py = getY() + height / 2.0 + dy * (height / 2.0 + 3);
        double speed = weapon.getProjectileSpeed();
        double vx = dx * speed;
        double vy = dy * speed;
        return new com.example.game.Projectile(px, py, vx, vy, weapon, root, Color.LIGHTGREEN, 5);
    }
}
